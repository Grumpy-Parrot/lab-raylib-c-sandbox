...
void main() {
    ...
    vec4 baseColor;
    baseColor.r = texture2D(texture0, vec2(remappedUV.x + separation, remappedUV.y)).r;
    baseColor.g = texture2D(texture0, remappedUV).g;
    baseColor.b = texture2D(texture0, vec2(remappedUV.x - separation, remappedUV.y)).b;
    baseColor.a = texture2D(texture0, remappedUV).a;
